...
- (UIScrollView *)pictureScrollView {
    if (!_pictureScrollView) {
        ...
        for (int i = 0; i < self.model.pics.count; i++) {
            ...
            [imageView sd_setImageWithURL:[NSURL OD_URLWithString:[NSString stringWithFormat:@"%@", self.model.pics[i]]]placeholderImage:[UIImage imageNamed:@"placeholderBigImage"] options:SDWebImageRetryFailed];
